...
export default function NotasTable() {
  ...
  const [tableData, setTableData] = useState(alumnosCurso ? alumnosCurso.map(alumno => ({
    ...
    // Only allow integer input for the first three fields
    ...
    newData[index].field4 = calculateAverage(newData[index].field1, newData[index].field2, newData[index].field3)

		newData[index].field7 = calculateAverage(newData[index].field1, newData[index].field2, newData[index].field3)

    setTableData(newData)
  }

	const isDisabled = (index: number, field: Fields) => {
    if (field === 'field4') return true // Always disable the average field
    if (['field5', 'field6', 'field7'].includes(field)) {
      const average = parseFloat(tableData[index].field4)
      return average >= 7
    }
    return false
  }
            ...
            {['field1', 'field2', 'field3', 'field4', 'field5', 'field6', 'field7'].map((field) => (
              <TableCell key={field}>
                <Input
                  ...
                  type={['field1', 'field2', 'field3'].includes(field) ? 'number' : 'text'}
                />
              </TableCell>
            ))}
          </TableRow>
